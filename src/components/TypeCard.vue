        ...
        <div style="top: 0px; left: 0px; position: absolute; width: 100%; height: 130px; z-index: 9; padding: 15px 20px;">
          <div class="date">
            <div class="date-title">
              <h2>
                <span class="date-number" style="font-size: 28px;">{{message.ptype}}</span>
                <span>平台数目</span>
...
<script>
  ...
  export default {
    ...
    created () {
      var platformTypeClassic = this.$store.state.platformTypeClassic
      console.log(this.contextData)
      if (tools.isEmptyDict(platformTypeClassic.data)) {
        let _this = this
        axiosGet('/platform_type_classic')
          .then(res => {
            _this.$store.commit('platformTypeClassic', [res.total, res.data])
        ...
        console.log(platformTypeClassic)
        this.total = platformTypeClassic.total
        this.contextData = platformTypeClassic.data
        // this.setEntryColor()
      }
    },
    methods: {
