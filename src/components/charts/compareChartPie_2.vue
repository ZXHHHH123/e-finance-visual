<template>
   <div id="contrstPieChart2" style="width: 100%;height: 100%;"></div>
</template>
...
export default {
  ...
  mounted: function () {
    this.chartCanvas = echarts.init(document.getElementById('contrstPieChart2'))
    this.chartOption = {
      baseOption: {
        timeline: {
          axisType: 'category',
      ...
      for (let i = 0, len = arr.length; i < len; i++) {
        ...
        for (let j = 0, len = curValue[arr[i]].length; j < len; j++) {
          ...
          keyValue.push({
            ...
            value: curValue[arr[i]][j].monthturnover
          })
        }
        off = false
